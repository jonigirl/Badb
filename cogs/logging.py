...
from discord.utils import check
import os
...
class dbHandler:
    ...
    def createTableIfNotExist(self):
        if self.conn.cursor().execute("SELECT COUNT(*) FROM sqlite_master WHERE type = 'table'").fetchone()[0] == 0:
            c = self.conn.cursor()
            c.execute('CREATE table {0} ({1} TEXT, {2} TEXT, {3} TEXT, {4} TEXT, {5} TEXT, {6} TEXT, {7} TEXT)'.format(self.DB_TABLE_LOG, self.DB_LOG_SERVER_NAME,
                                                                                                                       self.DB_LOG_CHANNEL_NAME, self.DB_LOG_MESSAGE_ID, self.DB_LOG_OPERATION, self.DB_LOG_MESSAGE_CONTENT, self.DB_LOG_AUTHOR_NAME, self.DB_LOG_TIMESTAMP))
            c.execute('CREATE table {0} ({1} TEXT, {2} TEXT, {3} TEXT, {4} TEXT, {5} TEXT, {6} TEXT, {7} TEXT, {8} TEXT)'.format(self.DB_TABLE_REACTION, self.DB_REACTION_SERVER_NAME,
                                                                                                                                 self.DB_REACTION_CHANNEL_NAME, self.DB_REACTION_OPERATION, self.DB_REACTION_MESSAGE_ID, self.DB_REACTION_MESSAGE, self.DB_REACTION_EMOJI, self.DB_REACTION_USER, self.DB_REACTION_TIMESTAMP))
    ...
    def addLog(self, server_name, message_id, operation, message_content, author_name, channel_name, timestamp):
        c = self.conn.cursor()
        ts = str(timestamp)
        c.execute("INSERT INTO {0} ({1}, {2}, {3}, {4}, {5}, {6}, {7}) VALUES (?, ?, ?, ?, ?, ?, ?)".format(self.DB_TABLE_LOG, self.DB_LOG_SERVER_NAME, self.DB_LOG_MESSAGE_ID, self.DB_LOG_OPERATION, self.DB_LOG_MESSAGE_CONTENT,
                                                                                                            self.DB_LOG_AUTHOR_NAME, self.DB_LOG_CHANNEL_NAME, self.DB_LOG_TIMESTAMP), (str(server_name), str(message_id), str(operation), str(message_content), str(author_name), str(channel_name), ts[:19]))
        self.conn.commit()
        return True
    ...
    def addReaction(self, server_name, message_id, operation, reaction_message, emoji, user, channel_name, timestamp):
        c = self.conn.cursor()
        ts = str(timestamp)
        c.execute("INSERT INTO {0} ({1}, {2}, {3}, {4}, {5}, {6}, {7}, {8}) VALUES (?, ?, ?, ?, ?, ?, ?, ?)".format(self.DB_TABLE_REACTION, self.DB_REACTION_SERVER_NAME, self.DB_REACTION_MESSAGE_ID, self.DB_REACTION_OPERATION, self.DB_REACTION_MESSAGE,
    ...
    def deleteOldLogReaction(self):
        c = self.conn.cursor()
        c.execute("DELETE FROM {0} WHERE ({1} <= datetime('now', '-1 month'))".format(
            self.DB_TABLE_LOG, self.DB_LOG_TIMESTAMP))
        c.execute("DELETE FROM {0} WHERE ({1} <= datetime('now', '-1 month'))".format(
            self.DB_TABLE_REACTION, self.DB_REACTION_TIMESTAMP))
...
class logging:
    ...
    async def on_message(self, message):
        ...
        content += "" if not message.attachments else str(
            message.attachments[0]['url'])
        self.db.addLog(message.server, message.id, self.OPERATION_MESSAGE,
                       content, message.author, message.channel, message.timestamp)
    ...
    async def on_message_edit(self, before, after):
        ...
        if before != after:
            ...
            content += "" if not after.attachments else str(
                after.attachments[0]['url'])
            self.db.addLog(after.server, after.id, self.OPERATION_EDIT,
                           content, after.author, after.channel, after.timestamp)
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        content = str(reaction.message.content) if len(
            reaction.message.content) > 0 else ""
        content += "" if not reaction.message.attachments else str(
