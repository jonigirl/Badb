...
def create_id(n: int = 5):
    ...
    return ''.join(random.choices(
        string.ascii_lowercase + string.digits, k=n)).lower()
...
class QuoteCommands(vbu.Cog):
    ...
    async def get_quote_messages(
        ...
        # 22 threshold 15 (threshold 15) mcabe
            self, ctx: vbu.Context, messages: typing.List[discord.Message], *,
            allow_self_quote: bool = False) -> dict:
        """
        Gets the messages that the user has quoted, returning a dict with \
        keys `success` (bool) and `message` (str or voxelbotutils.Embed).
        If `success` is `False`, then the resulting `message` can be directly \
        output to the user, and if it's `True` then we can go ahead
        ...
        if not messages:
            if ctx.message.reference is not None:
                ...
                return {'success': False, 'message': "I couldn't find any \
                references to messages in your command call."}
        ...
        for i, o in zip(messages, messages[1:]):
            ...
            if (o.created_at - i.created_at).total_seconds() > 3 * 60:
                return {'success': False, 'message': "Those messages are too \
                far apart to quote together."}
            if not i.content or i.attachments:
                if len(i.attachments) == 0:
                    return {'success': False, 'message': "Embeds can't \
                    be quoted."}
                if i.attachments:
                    return {'success': False, 'message': "You can't quote \
                    multiple messages when quoting images."}

        # Validate the message content
        for message in messages:
            if (quote_is_url and message.content) or (
                    message.content and message.attachments and
                    message.content != message.attachments[0].url):
                return {'success': False, 'message': "You can't quote \
                both messages and images."}
            elif message.embeds and getattr(
                message.embeds[0].thumbnail,
                    "url", None) != message.content:
        ...
        if len(set([i.author.id for i in messages])) != 1:
            return {'success': False, 'message': "You can only quote one \
            person at a time."}

        # Stops self quoting if there are no reactions needed
        message_author = messages[0].author
        reactions_needed = self.bot.guild_settings[ctx.guild.id]
        ['quote_reactions_needed']
        ...
        return {'success': True,
                'message': embed, 'user': user, 'timestamp': timestamp}
    ...
    async def quote(
        ...
        ask_to_save_message = await ctx.send(
            f"Should I save this quote? If I receive {reactions_needed} \
            positive reactions in the next 60 seconds, \
            the quote will be saved.",
            embed=embed,
        )
        ...
        try:
            ...
            reaction_count = sum(
                [i.count if str(i.emoji) == "\N{THUMBS UP SIGN}" else -i.count
                    ...
                    ask_to_save_message_again.reactions])
        ...
        if reaction_count < reactions_needed:
            return await ctx.send(f"_Not_ saving the quote asked \
                by {ctx.author.mention} - not enough reactions received.")
        ...
        if quote_channel_id is None or posted_message is None:
            return await ctx.send("I couldn't send your quote into the \
                quote channel.")
        ...
        async with vbu.Database() as db:
            await db(
                ...
                posted_message.id, user.id, timestamp, ctx.author.id,
            )

        # Output to user
        await ctx.send(f"{ctx.author.mention}'s quote request saved with \
            ID `{quote_id.upper()}`", embed=embed)

    @commands.context_command(name="Quote message")
    async def _context_command_quote_create(
    ...
    async def quote_force(
            self, ctx: vbu.Context, messages: commands.Greedy[discord.Message]):
        """
        Quotes a user's message to the guild's quote channel.
        """
        ...
        if ctx.invoked_subcommand is not None:
            return
        response = await self.get_quote_messages(
            ctx, messages, allow_self_quote=True)
        ...
        if quote_channel_id:
            ...
            try:
                ...
                pass
        if quote_channel_id is None or posted_message is None:
            return await ctx.send("I couldn't send your quote into the quote \
                channel.")
        ...
        async with vbu.Database() as db:
            await db(
                ...
                VALUES ($1, $2, $3, $4, $5, $6, $7)""",
                quote_id, ctx.guild.id, posted_message.channel.id,
                posted_message.id, user.id, timestamp.replace(
                    tzinfo=None), ctx.author.id,
            )

        # Output to user
        await ctx.send(f"{ctx.author.mention}'s quote saved with \
            ID `{quote_id.upper()}`", embed=embed)
    ...
    async def quote_get(self, ctx: vbu.Context, identifier: str):
        ...
        async with vbu.Database() as db:
            quote_rows = await db(
                """SELECT user_quotes.quote_id as quote_id, user_id, \
                channel_id, message_id FROM user_quotes LEFT JOIN
        ...
        if data['channel_id'] is None:
            return await ctx.send("There's no quote channel \
                set for that quote.")
        ...
        # try to refresh the user name and icon of the embed
        # by getting the user from the user ID in the DB
        quote_embed = message.embeds[0]
        quote_author = self.bot.get_user(data['user_id'])
        if quote_author:
            quote_embed.set_author(
                name=quote_author.display_name,
    ...
    async def quote_random(
        ...
        async with vbu.Database() as db:
            quote_rows = await db(
                ...
                FROM user_quotes WHERE user_id=$1 AND \
                guild_id=$2 ORDER BY RANDOM() LIMIT 1""",
        ...
        if data['channel_id'] is None:
            ...
            async with vbu.Database() as db:
                await db(
                    "DELETE FROM user_quotes WHERE quote_id=$1", data
                    ['quote_id'])
        ...
        if quote_author:
            quote_embed.set_author(
                name=quote_author
                .display_name, icon_url=quote_author.display_avatar.url)
        return await ctx.send(embed=quote_embed)
    ...
    # async def quote_alias(self, ctx: vbu.Context, quote_id: \
    # commands.clean_content, alias: commands.clean_content):
    #     """
    ...
    #     # Grab data from db
    #     async with vbu.Database() as db:
    #         rows = await db("SELECT * FROM user_quotes WHERE quote_id=$1 \
    # AND guild_id=$2", quote_id.lower(), ctx.guild.id)
    #     if not rows:
    #         return await ctx.send(f"There's no quote with the ID \
    # `{quote_id.upper()}`.")

    #     # Insert alias into db
    #     async with vbu.Database() as db:
    #         rows = await db("SELECT * FROM quote_aliases WHERE \
    # alias=$1", alias)
    #         if rows:
    #             return await ctx.send(f"The alias `{alias}` \
    # is already being used.")
    #         await db("INSERT INTO quote_aliases (quote_id, alias) \
    # VALUES ($1, $2)", quote_id.lower(), alias.lower())
    #     await ctx.send(f"Added the alias `{alias.upper()}` to quote \
    # ID `{quote_id.upper()}`.")
    ...
    #     user = user or ctx.author
    #     async with vbu.Database() as db:
    #         rows = await db("SELECT quote_id FROM user_quotes WHERE \
    # user_id=$1 AND guild_id=$2", user, ctx.guild.id)
    #     if not rows:
    #         embed = vbu.Embed(
    #             use_random_colour=True, description="This user \
    # has no quotes.",
    ...
    # @commands.has_guild_permissions(manage_guild=True)
    # @commands.bot_has_permissions(send_messages=True)
    # async def quote_alias_remove(self, ctx: vbu.Context, \
    ...
    #         )
    #         if not quote_rows:
    #             return await ctx.send(f"There's no quote with the alias \
    # `{alias.upper()}`.")
    #         await db("DELETE FROM quote_aliases WHERE \
    # alias=$1", alias.lower())
    ...
    async def quote_delete(self, ctx: vbu.Context, quote_id: str):
        ...
        quote_channel_id = self.bot.guild_settings[
            ctx.guild.id].get('quote_channel_id')
        if quote_channel_id:
        ...
        async with vbu.Database() as db:
            await db(
                "DELETE FROM user_quotes WHERE quote_id=ANY($1) \
                AND guild_id=$2", quote_ids, ctx.guild.id)
