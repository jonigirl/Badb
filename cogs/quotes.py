...
class QuoteCommands(vbu.Cog):
        ...
        *,
        ...
        for i, o in zip(messages, messages[1:]):
            ...
            if (o.created_at - i.created_at).total_seconds() > 3 * 60:
                return {
                    "success": False,
                    "message": "Those messages are too \
            ...
            if not i.content or i.attachments:
                if len(i.attachments) == 0:
                    return {
                        "success": False,
                        "message": "Embeds can't \
                    be quoted.",
                    }
                if i.attachments:
                    return {
                        "success": False,
                        "message": "You can't quote \
        ...
        for message in messages:
            if (quote_is_url and message.content) or (
                ...
                message.embeds
                and getattr(message.embeds[0].thumbnail, "url", None) != message.content
            ):
        ...
        if len(set([i.author.id for i in messages])) != 1:
            return {
                "success": False,
                "message": "You can only quote one \
            person at a time.",
            }
        ...
        if ctx.author.id in self.bot.owner_ids:
            ...
            return {
                "success": False,
                "message": "You can't quote yourself \
    ...
    async def quote(self, ctx: vbu.Context, messages: commands.Greedy[discord.Message]):
        ...
        try:
            ...
            reaction_count = sum(
                [
                    i.count
                    if str(i.emoji) == "\N{THUMBS UP SIGN}"
                    else -i.count
                    if str(i.emoji) == "\N{THUMBS DOWN SIGN}"
                    else 0
        ...
        if reaction_count < reactions_needed:
            return await ctx.send(
                f"_Not_ saving the quote asked \
        ...
        if quote_channel_id is None or posted_message is None:
            return await ctx.send(
                "I couldn't send your quote into the \
                quote channel."
            )
        ...
        # Output to user
        await ctx.send(
            f"{ctx.author.mention}'s quote request saved with \
            ID `{quote_id.upper()}`",
            embed=embed,
        )
    ...
    async def quote_force(
        ...
        if quote_channel_id is None or posted_message is None:
            return await ctx.send(
                "I couldn't send your quote into the quote \
                channel."
            )
        ...
        # Output to user
        await ctx.send(
            f"{ctx.author.mention}'s quote saved with \
            ID `{quote_id.upper()}`",
            embed=embed,
        )
    ...
    async def quote_get(self, ctx: vbu.Context, identifier: str):
        ...
        if quote_author:
            quote_embed.set_author(
                name=quote_author.display_name, icon_url=quote_author.display_avatar.url
            )
