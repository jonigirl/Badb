...
class Warn(vbu.Cog):
    ...
    async def warn_command(self, ctx, user: discord.Member, *, reason: str):
        if user.id == self.bot.user.id:
            ...
            await ctx.send(
                "Oh, REALLY now, huh? I do my best at maintaining this server and THIS is how you treat me? Screw this.."
            )
            return
        if user.bot == 1:
        ...
        try:
            ...
            with open(f"data/warns/{str(ctx.guild.id)}/{str(user.id)}.json", "w") as f:
                ...
                json.dump(data, f)
            embed = discord.Embed(
                title=f"{user.name}'s new warn", color=random.choice(color_list)
            )
            embed.set_author(
        ...
        json.dump(
            data, open(f"data/warns/{str(ctx.guild.id)}/{str(user.id)}.json", "a")
        )
    ...
    async def warn_handler(self, ctx, error):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            if error.param.name == "user":
                ...
                await ctx.send(
                    "{0.author.name}, you forgot to specify a user to warn. *(commands.MissingRequiredArgument error, action cancelled)*".format(
                        ctx
                    )
                )
                return
    ...
    async def warns_command(self, ctx, user: discord.Member):
        try:
            ...
            await ctx.send(
                f"{ctx.author.name}, user [{user.name} ({user.id})] does not have any warns."
            )
            return
    ...
    async def remove_warn_command(self, ctx, user: discord.Member, *, warn: str):
        ...
        if reply in ("y", "yes", "confirm"):
            ...
            if warn_amount == 1:  # Check if the user only has one warn.
                os.remove(
                    "data/warns/" + str(ctx.guild.id) + "/" + str(user.id) + ".json"
                )  # Removes the JSON containing their only warn.
                await ctx.send(
            ...
            if (
                warn != warn_amount
            ):  # Check if the warn to remove was not the last warn.
    ...
    async def edit_warn_command(self, ctx, user: discord.Member, *, warn: str):
        ...
        msg = await self.bot.wait_for(
            "message", check=check
        )  # Pause the ongoing script to wait for the user to send a message.
        ...
        await ctx.send(
            content="Are you sure you want to edit this warn like this? (Reply with y/yes or n/no)",
            embed=confirmation_embed,
        )
        ...
        if reply in ("y", "yes", "confirm"):
            ...
            await ctx.send(
                f"[{ctx.author.name}], user [{user.name} ({user.id})] has gotten their warn edited."
            )
            return
