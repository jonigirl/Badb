...
class UserPoints(vbu.Cog):
    ...
    async def points_add(
        self, ctx: vbu.Context, user: typing.Optional[discord.Member], points: int = 1
    ):
        """
        ...
        # Send output
        await ctx.send(
            f"Added {points} points to {user.mention}.",
    ...
    async def on_leaderboard_update(self, guild: discord.Guild):
        ...
        leaderboard_message_url = self.bot.guild_settings[guild.id][
            "leaderboard_message_url"
        ]
