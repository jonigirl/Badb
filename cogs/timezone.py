...
class TimezoneInfo(vbu.Cog):
    ...
    async def timezone_set(self, ctx: vbu.Context, *, offset: str = None):
        ...
        await ctx.send(
            f"I think your current time is **{discord.utils.utcnow().astimezone(zone).strftime('%-I:%M %p')}** - I've stored this in the database."
        )
    ...
    async def timezone_get(
        self, ctx: vbu.Context, target: typing.Union[discord.Member, str] = None
    ):
        """
        ...
        if not target_is_timezone:
            async with vbu.Database() as db:
                rows = await db(
                    "SELECT timezone_name, timezone_offset FROM user_settings WHERE user_id=$1",
                    target.id,
                )
            if not rows or (
                rows[0]["timezone_name"] is None and rows[0]["timezone_offset"] is None
            ):
                return await ctx.send(
                    f"{target.mention} hasn't set up their timezone information! They can set it by running `{ctx.clean_prefix}timezone set`."
                )
        ...
        if target_is_timezone:
            try:
                formatted_time = (
                    discord.utils.utcnow().astimezone(pytz.timezone(target))
                ).strftime("%-I:%M %p")
            except pytz.UnknownTimeZoneError:
                return await ctx.send("That isn't a valid timezone.")
            return await ctx.send(
                f"The current time in **{target}** is estimated to be **{formatted_time}**."
            )
        elif rows:
            if rows[0]["timezone_name"]:
                formatted_time = (
                    discord.utils.utcnow().astimezone(
                        pytz.timezone(rows[0]["timezone_name"])
                    )
                ).strftime("%-I:%M %p")
            else:
                formatted_time = (
            ...
            await ctx.send(
                f"The current time for {target.mention} is estimated to be **{formatted_time}**.",
