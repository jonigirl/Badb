import discord
from discord.ext import commands, vbu, send_cmd_help, settings
from discord.utils import checks
from discord.utils.dataIO import fileIO
from bot import send_cmd_help, settings
import os
import logging
...
class Mod(vbu.Cog):
    ...
    @vbu.group(pass_context=True, no_pm=True)
    ...
    @vbu.group(name="filter", pass_context=True, no_pm=True)
    @checks.mod_or_permissions(manage_messages=True)
    async def _filter(self, ctx):
        """Adds/removes words from filter

        Use double quotes to add/remove sentences
        Using this command with no subvbu will send
    ...
    @vbu.group(no_pm=True, pass_context=True)
